#Cavium implementation
import random

reg_A = [''] * 93
reg_B = [''] * 84
reg_C = [''] * 111

treg_A = [''] * 93
treg_B = [''] * 84
treg_C = [''] * 111

ca_rule=[1,2,3,4,5,6,7,8]
#<30,60,90,120,150,180,210,240>

def parse_reg(ind,n):
    if n==93:
        if ind==0:
            return [0,reg_A[ind],reg_A[ind+1]]
        elif ind==92:
            return [reg_A[ind-1],reg_A[ind],0]
        else:
            return [reg_A[ind-1],reg_A[ind],reg_A[ind+1]]
    if n==84:
        if ind==0:
            return [0,reg_B[ind],reg_B[ind+1]]
        elif ind==83:
            return [reg_B[ind-1],reg_B[ind],0]
        else:
            return [reg_B[ind-1],reg_B[ind],reg_B[ind+1]]
    if n==111:
        if ind==0:
            return [0,reg_C[ind],reg_C[ind+1]]
        elif ind==110:
            return [reg_C[ind-1],reg_C[ind],0]
        else:
            return [reg_C[ind-1],reg_C[ind],reg_C[ind+1]]

def ca(a,b,c,n):
    #rule 30
    if n==1:
        return (1^a)&c | (1^c)&(a^b)
    #rule 60
    elif n==2:
        return a^b
    #rule 90
    elif n==3:
        return a^c
    #rule 120
    elif n==4:
        return c&(a^b) | a&(1^c)
    #rule 150
    elif n==5:
        return a^b^c
    #rule 180
    elif n==6:
        return a&c | (1^c)&(a^b)
    #rule 210
    elif n==7:
        return (1^b)&(a^c) | a&b
    #rule 240
    elif n==8:
        return a


for i in [93, 84, 111]:
    for j in range(i):
        if i == 93:
            reg_A[j] = 0
        elif i == 84:
            reg_B[j] = 0
        elif i == 111:
            reg_C[j] = 0

reg_C[110] = reg_C[109] = reg_C[108] = 0

'''for i in range(80):
    reg_A[i] = random.getrandbits(1)
    reg_B[i] = random.getrandbits(1)
'''
str1 = '00101110110000000110010101111101000011011100110100101111011001010101110010011001'
str2 = '01001111010101010111110111010110101011110010110100001100010000010111101100100000'

#str1 is key and str2 is IV
for i in range(80):  # initializing key and IV
    reg_A[i] = int(str1[i])
    reg_B[i] = int(str2[i])

str1 = ''.join(list(map(str,reg_A[0:79])))
str2 = ''.join(list(map(str,reg_B[0:79])))

print ('Key:', end = ' ')
print (str1)
print ('\nIV: ', end = ' ')
print (str2)
print ()


stream = []
for i in range(100000144):
    t1 = reg_A[65] ^ reg_A[92]
    t2 = reg_B[68] ^ reg_B[83]
    t3 = reg_C[65] ^ reg_C[110]

    T1 = t1 ^ (reg_A[90] & reg_A[91]) ^ reg_B[76]
    T2 = t2 ^ (reg_B[81] & reg_B[82]) ^ reg_C[87]
    T3 = t3 ^ (reg_C[109] & reg_C[110]) ^ reg_A[68]
    for x in range(92,0,-1):
        reg_A[x] = reg_A[x-1]
        a,b,c = parse_reg(x-1,93)
        treg_A[x] = ca(a,b,c,ca_rule[(x%8)-1])
    treg_A[0]=T1
    reg_A[0] = T1
    for x in range(83,0,-1):
        reg_B[x] = reg_B[x-1]
        a,b,c = parse_reg(x-1,84)
        treg_B[x] = ca(a,b,c,ca_rule[(x%8)-1])
    treg_B[0]=T2
    reg_B[0] = T2
    for x in range(110,0,-1):
        reg_C[x] = reg_C[x-1]
        a,b,c = parse_reg(x-1,111)
        treg_C[x] = ca(a,b,c,ca_rule[(x%8)-1])
    treg_C[0]=T3
    reg_C[0] = T3


    reg_A=treg_A
    reg_B=treg_B
    reg_C=treg_C


    if(i > 143):
        stream.append(t1^t2^t3)


bits = ''.join(list(map(str,stream)))
#print()
#11print(bits,end = ' After 1000 clock cycles\n')

f = open('data/cav_stream.txt','wt')
f.write(bits)
